...
7. **Given** the `--no-update-check` flag is passed, **When** the developer runs `spectra version`, **Then** the GitHub network call for Spectra CLI latest version is suppressed (that row reports "unknown" or just the installed version), but local checks (specify self check, integration.json) still run.
...
- **FR-016**: The `--no-update-check` flag MUST suppress GitHub network calls for the Spectra CLI latest version, but MUST NOT suppress local checks (specify self check, integration.json reads).
