...
- **FR-011**: The command MUST populate the BRD's Document Control fields (identifier, title, author, status = Draft, version, created / last-updated dates) from context.
- **FR-012**: After writing, the command MUST report the output file path and instruct the user that they can now run `/speckit-specify` with the constructed BRD; it MUST NOT invoke `specify` itself.
